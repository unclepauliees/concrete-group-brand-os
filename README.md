...
A fixed top-right `ThemeToggle` switches between a Light version (bone throughout) and a Dark version (ink throughout) of the whole site — every section shares one `theme` value (`dark ? "ink" : "bone"`) rather than each section keeping its own fixed ground and inverting individually, which read as an unpredictable mix of light and dark sections regardless of which mode was selected. House green is the one exception: the Logo section stays green in both themes, since it's demonstrating the mark's approved ground, not narrating the page's own light/dark state. The choice persists to `localStorage` (`tcg-dark`) so a reload doesn't reset to Light.

The root container's own background is set from the same `theme` value (`apps/guidelines/src/App.tsx`), not left to the page's CSS default. The `lg:pl-96` gutter reserved for the fixed rail sits outside every section's own background, so without this the gutter stayed bone even in Dark mode — a light strip running the full height of an otherwise-dark page.
